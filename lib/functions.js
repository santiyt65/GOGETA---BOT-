...
export async function handleCommand(sock, m) {
  ...
  // Detectamos si viene respuesta de botón:
  const buttonResponse = m.message?.buttonsResponseMessage?.selectedButtonId || "";
  // Si hay botón, usamos eso, sino usamos texto normal:
...
`.trim();
    ...
    } else if (command === ".ppt" || command === "piedra" || command === "papel" || command === "tijera") {
      // Pasamos el comando y el mensaje al plugin para que lo procese
...
async function cargarPlugins() {
  try {
    ...
    return { adivinaCommand, pptCommand, tagCommand };
  } catch (e) {
